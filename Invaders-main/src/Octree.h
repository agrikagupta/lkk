...
public:
	...
	void draw(TreeNode & node, int numLevels, int level, const vector<ofColor> & levelColors={});
	void draw(int numLevels, int level, const vector<ofColor> & levelColors={}) {
		draw(root, numLevels, level, levelColors);
	}
