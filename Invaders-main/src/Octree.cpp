...
void Octree::subdivide(const ofMesh & mesh, TreeNode & node, int numLevels, int level) {
	...
	// 1) Subdivide box into 8 equal boxes
	subDivideBox8(node.box, boxList);

	// 2) For each child box, sort point data and add to tree if contains points
	for (int i = 0; i < boxList.size(); i++) {
		vector<int> pointsInBox;
		int count = getMeshPointsInBox(mesh, node.points, boxList[i], pointsInBox);
		...
		if (count > 0) {
			...
			// If not a leaf node (more than 1 point), recursively subdivide
			if (count > 1) {
...
bool Octree::intersect(const Ray &ray, const TreeNode & node, TreeNode & nodeRtn) {
	if (!node.box.intersect(ray, 0, 10000))
		return false;
	if (node.children.size() == 0 && node.points.size() > 0) {
		nodeRtn = node;
		return true;
	}
...
void Octree::draw(TreeNode & node, int numLevels, int level, const vector<ofColor> & levelColors) {
	...
	level++;
	for (int i = 0; i < node.children.size(); i++) {
		draw(node.children[i], numLevels, level, levelColors);
	}
}
